...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(String xx1:tokens)
		{
			if(isWord( xx1)==true)
			{
				...
				boolean flag=false;
				int l=xx1.length();
				String wor="aiouyAIOUY";
				
				int count=0;
				for(int i=0;i<l;i++)
				{
					String let=xx1.substring(i,i+1);
					if("aiouyAIOUY".contains(let) && flag==false ||"eE".contains(let)  &&  i!=l-1 && flag==false )
					...
					else if("eE".contains(let) && count==0 && flag==false  && i==l-1 )
					{
						count++;
						flag=true;
					}
					else if(!"aeiouyAEIOUY".contains(let) && flag==true)
						flag=false;
				}
				numSyllables+=count;
				...
				{
					numSentences+=1;
					//System.out.println(numSentences+ "  "+(l1-1)+ "/t");
				}
			}
				else
				{
	...
	public int getNumWords() {
		...
		/*List<String> x3 = getTokens("[a-zA-Z]+");
	    return x3.size();*/
